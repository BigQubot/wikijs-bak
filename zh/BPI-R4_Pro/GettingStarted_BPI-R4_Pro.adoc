...
* Windows/Ubuntu 系统 +
* link:https://docs.banana-pi.org/en/BPI-R4_Pro/BananaPi_BPI-R4_Pro#_system_image[BPI-R4 Pro 镜像]
* link:https://docs.banana-pi.org/en/BPI-R4_Pro/BananaPi_BPI-R4_Pro#_source_code[BPI-R4 Pro 源代码]
...
== 烧录镜像
如需将出厂默认系统更换为其他系统，+
请在 Windows/Ubuntu 电脑上link:https://docs.banana-pi.org/en/BPI-R4_Pro/BananaPi_BPI-R4_Pro#_system_image[下载镜像]，然后按以下步骤操作。+
...
烧录 NAND 镜像前请准备一个 USB 驱动器。以下以 OpenWrt 镜像（**BPI-R4Pro-8X-BE14-MT76-OpenWRT24.10-DSA-snand-251120.bin**）为例：

. 将 USB 驱动器格式化为 FAT32。
. 将用于 NAND 启动的 OpenWrt 镜像（**BPI-R4Pro-8X-BE14-MT76-OpenWRT24.10-DSA-snand-251120.bin**）复制到 USB 驱动器。
...
```
. 关闭 BPI-R4 Pro，拔出 USB 驱动器，切换为从 NAND 启动，然后重新上电。
...
. 将 USB 驱动器格式化为 FAT32。
. 将用于 eMMC 启动的 OpenWrt 镜像（**bl2_emmc-r4pro.img, mtk-bpi-r4pro-EMMC-20250928.img**）复制到 USB 驱动器，+
如果镜像已压缩，请先解压再复制。
...
. 关闭 BPI-R4 Pro，拔出 USB 驱动器，切换为从 eMMC 启动，然后重新上电。
...
link:https://docs.banana-pi.org/en/BPI-R4/BananaPi_BPI-R4#_10g_sfp_module[已测试的 10G SFP+ 电口/光口模块] +
...
. 首先，将需要更新的 MxL 文件上传到开发板。可以通过 TFTP 或 USB 驱动器复制。
...
link:https://docs.banana-pi.org/en/BPI-R4/BananaPi_BPI-R4-NIC-BE14[BPI Wi-Fi 网卡：BE14 规格]
