...
== Powered 
There is another connector between the GPIO terminals and the Type-C (USB SERIAL) connector, which connects to the DC power socket.

You can directly power the board using a power cable connected to this socket.

PS: 
(1)The cable must be 0.5 mm² (AWG20).

(2)Also, do not connect the positive and negative terminals incorrectly.

(3)Do not power the board together with the DC power socket.

image::/bpi-f4/bpi-f4_power_socket.png[bpi-f4_power_socket.png]
...
Three jump caps are pre-installed on the board when it leaves the factory.

== Heat sink

If you need to use the AI function after purchasing the product, please purchase a heatsink to prevent the development board from overheating.
