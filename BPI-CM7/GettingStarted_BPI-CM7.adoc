...
. Power on and boot using SD card.

. Copy the Linux image to be burned to the USB drive.

. Connect the USB flash drive to the USB port and mount it using the mount command.
+
```bash
root@bpi-cm7:~# mount /dev/sda1 /mnt
root@bpi-cm7:~# lsblk
NAME         MAJ:MIN RM   SIZE RO TYPE MOUNTPOINTS
sda            8:0    1  59.5G  0 disk
└─sda1         8:1    1  59.5G  0 part /mnt
mmcblk0      179:0    0  29.3G  0 disk
mmcblk0boot0 179:16   0     4M  1 disk
mmcblk0boot1 179:32   0     4M  1 disk
mmcblk1      179:48   0  59.5G  0 disk
└─mmcblk1p1  179:49   0  58.9G  0 part /var/log.hdd
                                       /
zram0        254:0    0 982.5M  0 disk [SWAP]
zram1        254:1    0    50M  0 disk /var/log
```

. Use the cd tool to navigate to the storage image directory of the USB drive.Use the dd command to burn it into the EMMC.
+
```bash
root@bpi-cm7:/mnt# dd if=“image.name” of=/dev/mmcblk0
17547264+0 records in
17547264+0 records out
8984199168 bytes (9.0 GB, 8.4 GiB) copied, 427.645 s, 21.0 MB/s
```

. Then power off, remove the SD card, and you can boot using eMMC.
