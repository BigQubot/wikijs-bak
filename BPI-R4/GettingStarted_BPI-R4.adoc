...
== Upgrade ATF BL2
NOTE: When upgrading BL2, please confirm whether it is compatible with the board hardware and version.

First, select the Upgrade ATF BL2 of the bootmenu.

image::/bpi-r4/bpi-r4_bl2_upgrade1.jpg[bpi-r4_bl2_upgrade1.jpg]

Select the upgrade tool you use on the upgrade page to upgrade.

image::/bpi-r4/bpi-r4_bl2_upgrade2.jpg[bpi-r4_bl2_upgrade2.jpg]

=== TFTP

Install TFTP software on the PC and connect R4 LAN to the computer network port.Then change your computer's configuration based on the Settings, writing the updated filename without pressing Enter.

image::/bpi-r4/bpi-r4_bl2_upgrade3.jpg[bpi-r4_bl2_upgrade3.jpg]
image::/bpi-r4/bpi-r4_bl2_upgrade4.jpg[bpi-r4_bl2_upgrade4.jpg]

Open TFTP software on the PC, set the transfer file path and the ip of the PC.

image::/bpi-r4/bpi-r4_bl2_upgrade5.jpg[bpi-r4_bl2_upgrade5.jpg]

R4 board serial port debugging to confirm the transfer of BL2 file.

image::/bpi-r4/bpi-r4_bl2_upgrade6.jpg[bpi-r4_bl2_upgrade6.jpg]

After successfully upgrading the BL2 file, restart the device.

=== Xmodem/Ymodem

You need to use a serial port utility that can be transferred using Xmodem and Ymodem. (Xmodem operates similarly to Ymodem; only Xmodem is shown here.)

image::/bpi-r4/bpi-r4_bl2_upgrade_xmodem1.jpg[bpi-r4_bl2_upgrade_xmodem1.jpg]

Select Send Xmodem, then select the file you want to transfer, and click Send.

image::/bpi-r4/bpi-r4_bl2_upgrade_xmodem2.jpg[bpi-r4_bl2_upgrade_xmodem2.jpg]

When the transfer is complete, restart the device.
...
WARNING: **The image files we release use a random MAC address on each boot and do not utilize EEPROM. If you wish to use EEPROM, you will need to write a script to read the data and update the port.**
...
NOTE: At present, the driver supports 6G AP, but the open source nmcli toolkit does not support 6G AP, waiting for the open source organization to update, 6G STA can support.
...
==== STA
NOTE: 5G and 6G are similar to 2.4G, so only 2.4G connections are demonstrated here.
...
NOTE: SSID is the name of your WiFi
```bash
