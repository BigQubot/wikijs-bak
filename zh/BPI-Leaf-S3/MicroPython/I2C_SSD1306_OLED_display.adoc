SSD1306 OLED screen模块是一个非常常见的screen模块，可以使用 I2C 通信协议。最大可输出128张图像*64位，无灰度，单个像素只有开和关两种状态。控制逻辑比较简单，非常适合入门。学习单片机行车屏显示项目。

== 外部硬件要求

SSD1306 OLED 带I²C接口的屏模块，最好是128*64 像素。

== 驱动库下载

TIP: https://github.com/micropython/micropython-lib/blob/master/micropython/drivers/display/ssd1306/ssd1306.py[micropython/ssd1306.py 司机]

下载后 ssd1306.py 本地，上传到 MicroPython 设备。
