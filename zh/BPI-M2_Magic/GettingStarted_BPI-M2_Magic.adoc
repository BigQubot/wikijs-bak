...
TIP: 更多信息：link:/zh/BPI-M2_Magic/BananaPi_BPI-M2_Magic[Banana Pi BPI-M2_Magic]
...
+
NOTE: USB 接口旁的三针 UART0 在 BPI 发布的默认镜像中不能用作调试串口，因为 SD 卡与 UART0 复用相同的引脚，而默认镜像需要同时支持 SD 卡和 eMMC。如果只从 eMMC 启动，并希望启用 UART0 作为调试串口且不需要 SD 卡支持，请参阅 link:/zh/BPI-M2_Magic/GettingStarted_BPI-M2_Magic#_为仅使用_emmc_的镜像启用_uart0_调试[为仅使用 eMMC 的镜像启用 UART0 调试]。
+
