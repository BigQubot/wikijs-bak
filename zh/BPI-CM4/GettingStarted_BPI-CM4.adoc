...
TIP: 更多信息： link:/zh/BPI-CM4/BananaPi_BPI-CM4[Banana Pi BPI-CM4]
...
. SoC ROM 首先尝试从 eMMC 启动。因此，只要 eMMC 中存在可启动镜像，开发板就无法从 SD 卡启动。更多信息请参阅开发板的 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_启动顺序[启动顺序]。
...
. 使用 SurfaceFlinger 旋转，需要 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_构建_android_源代码[修改 Android 源代码并重新构建]
+
...
. 修改 /boot/env.txt 中的 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_dtb_overlay[DTB Overlay] 配置，启用摄像头 DTBO。
+
```sh
 overlays="os08a10"
```
...
. 构建并安装 WiringPi。在 Debian 上构建之前，必须先 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_在_debian_中启用_sudo[安装 sudo]。
+
```sh
 $ sudo apt-get update
...
lcd-bpi：link:https://docs.banana-pi.org/zh/BPI-LCD_1602/BananaPi_BPI-LCD_1602[BPI LCD 1602 显示模块]示例。 +
52pi-bpi：link:https://docs.banana-pi.org/zh/BPI-OLED/BananaPi_BPI-OLED[BPI OLED 显示模块]示例。 +
matrixled-bpi：link:https://docs.banana-pi.org/zh/BPI-RGB_LED/BananaPi_BPI_RGB_LED[BPI RGB LED 矩阵扩展模块]示例。 +
berryclip-bpi：link:https://docs.banana-pi.org/zh/BPI-BerrClip/BananaPi_BPI-BerryClip[BPI BerryClip 模块]示例。

== RPi.GPIO
构建并安装 RPi.GPIO。在 Debian 上构建之前，必须先 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_在_debian_中启用_sudo[安装 sudo]。
```sh
 $ sudo apt-get update
...
== WiringPi2-Python
构建并安装 WiringPi2-Python。在 Debian 上构建之前，必须先 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_在_debian_中启用_sudo[安装 sudo]。
```sh
 $ sudo apt-get update
...
. 最简单的方法是将 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_将镜像写入_sd_卡[CM4 Linux 镜像写入 SD 卡或 eMMC]，用于引导系统；同时将该镜像写入 NVMe SSD，用于加载根文件系统。
. 使用 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_启动顺序[SD 卡或 eMMC] 启动 CM4，然后修改 /boot/boot.ini，让内核从 NVMe 分区加载根文件系统。
+
```sh
...
```
以上两种方法实际只擦除 eMMC 中 Android 的引导加载程序部分。从 SD 卡启动 Linux 后，建议再 link:/zh/BPI-CM4/GettingStarted_BPI-CM4#_使用_dd_命令擦除_emmc_中的_android_数据[使用 dd 命令格式化整个 eMMC]。
