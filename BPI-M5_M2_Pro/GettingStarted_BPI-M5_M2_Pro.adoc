...
lcd-bpi, link:/en/BPI_LCD_1602_display_module[BPI LCD 1602 display module] example. +
52pi-bpi, link:/en/BPI_OLED_Display_Module[BPI OLED Display Module] example. +
matrixled-bpi, link:/en/BPI_RGB_LED_Matrix_Expansion_Module[BPI RGB LED Matrix Expansion Module] example. +
berryclip-bpi, link:/en/BPI_BerryClip_Module[BPI BerryClip Module]

=== RPi.GPIO

Build and install, for debian, you must link:/en/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_enable_sudo_for_debian[install sudo] before build
```sh
 $ sudo apt-get update
...
. The simple way is link:/en/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_install_image_to_sdcard[flash the M5/M2Pro Linux image to sdcard or emmc] for bootable and also flash it to the usb drive for loading boot and rootfs.
. Bootup the M5/M2pro board with link:/en/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_boot_sequence[sdcard or emmc], copy /boot/boot.ini to /boot/boot.ini.org so that bootloader load boot.ini fail and then try to load it from usb drive boot partition.
...
SDIO/UART type: Connect the hardware module to 40pin header correctly and link:/en/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_android_dtb_overlay[configure the Android DTB overlay] to enable it.
+
