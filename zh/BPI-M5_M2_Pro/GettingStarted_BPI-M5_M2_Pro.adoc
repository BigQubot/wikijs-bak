...
TIP: 更多信息：link:/zh/BPI-M5/BananaPi_BPI-M5[Banana Pi BPI-M5] +
更多信息：link:/zh/BPI-M2_Pro/BananaPi_BPI-M2_Pro[Banana Pi BPI-M2 Pro]
...
lcd-bpi：link:/zh/BPI-LCD_1602/BananaPi_BPI-LCD_1602[BPI LCD 1602 显示模块]示例。 +
52pi-bpi：link:/zh/BPI-OLED/BananaPi_BPI-OLED[BPI OLED 显示模块]示例。 +
matrixled-bpi：link:/zh/BPI-RGB_LED/BananaPi_BPI_RGB_LED[BPI RGB LED 矩阵扩展模块]示例。 +
berryclip-bpi：link:/zh/BPI-BerrClip/BananaPi_BPI-BerryClip[BPI BerryClip 模块]示例。

=== RPi.GPIO

构建并安装 RPi.GPIO。在 Debian 上构建之前，必须先 link:/zh/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_在_debian_中启用_sudo[安装 sudo]。
```sh
 $ sudo apt-get update
...
构建并安装 WiringPi2-Python。在 Debian 上构建之前，必须先 link:/zh/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_在_debian_中启用_sudo[安装 sudo]。
```sh
 $ sudo apt-get update
...
. 最简单的方法是将 link:/zh/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_将镜像写入_sd_卡[BPI-M5/BPI-M2 Pro Linux 镜像写入 SD 卡或 eMMC] 用于引导，同时将该镜像写入 U 盘，用于加载 boot 和根文件系统。
. 使用 link:/zh/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_启动顺序[SD 卡或 eMMC] 启动 BPI-M5/BPI-M2 Pro，将 /boot/boot.ini 复制为 /boot/boot.ini.org，使引导加载程序无法在该介质加载 boot.ini，继而尝试从 U 盘启动分区加载。
...
SDIO/UART 类型：将硬件模块正确连接到 40 Pin 排针，并 link:/zh/BPI-M5_M2_Pro/GettingStarted_BPI-M5_M2_Pro#_android_dtb_overlay[配置 Android DTB Overlay] 以启用该模块。
+
