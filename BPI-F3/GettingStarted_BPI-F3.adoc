...
WARNING: Please select the image ending in "**.img.zip**" for **Bianbu**. Don't choose the wrong one! +
...
WARNING: Please select the image ending in "**.zip**" for **Bianbu**. Don't choose the wrong one! +
The **Armbian** image currently only supports SD card startup.
...
. Copy the image to a USB drive and insert F3.(Taking “**bianbu-23.10-nas-k1-v1.0rc1-release-20240429192450**“ as an example)
...
+
```sh
sudo dd if=bianbu-23.10-nas-k1-v1.0rc1-release-20240429192450 of=/dev/mmcblk2 bs=10M
```
