...
== RTC
NOTE: After the system is powered on, the RTC will automatically synchronize the system time. You can use commands to query the system time and the RTC time. The system time here is CST time, while the RTC shows UTC time. The difference of 8 hours is within the normal range.

image::/bpi-f3/banana_pi_bpi-f3_rtc1.jpg[banana_pi_bpi-f3_rtc1.jpg]
. Connect the battery to the RTC interface of F3.Then Connect the power supply of BPI-F3 and turn on the power to start the device.Connect to the network to obtain real-time time.
+
```sh
root@k1:~# date
