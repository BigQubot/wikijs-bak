...
. Copy Nand bootable and EMMC boot OpenWrt image(**mtk-bpi-r3-NAND-WAN1-SFP1-20220619-single-image.bin**) to USB disk, if the image is compressed please uncompress it before copying to USB disk.
...
```
. Remove SD card, and change bootstrap to boot from nand flash and change SD/EMMC switch jumper to NAND, restart the board from Nand Flash.
+
