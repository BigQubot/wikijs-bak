...
This source code is no longer compatible due to a change in the WiFi chip's packaging.
...
For PSB boards with silkscreen markings of XXX or later, please use the TGP 3.3.4 version image file, which is pre-programmed at the factory. (Previous batches of development boards are also compatible with older chips.)
