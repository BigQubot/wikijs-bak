...
Bianbu Desktop is a simple, fast and secure desktop operating system that supports RISC-V single-board computers, notebooks and desktops.

=== Bianbu NAS

Bianbu NAS is a NAS operating system that includes applications (openmediavault, docker, and common software), frameworks, libraries, runtime, Linux kernel, boot loader (U-Boot), and supervisor interface (OpenSBI). The goal is to provide customers with NAS software reference designs, and drivers or applications can be developed.

Bianbu Desktop/NAS image download： https://archive.spacemit.com/image/k1/version/bianbu/
...
Spcemit official online documents : https://bianbu-linux.spacemit.com/openwrt-docs
