...
- The following example code performs an SPI loopback test. Please directly connect 40-pin physical pin 19 (SPI2_MOSI) to physical pin 21 (SPI2_MISO). You need to enable the spi2 DTB overlay before the test.
+
```sh
...
(myenv) pi@bpi-ai2n:~/project$
```

. Install gpiod python binding library
+
```sh
(myenv) pi@bpi-ai2n:~/project$ pip3 install gpiod
```
...
+
```sh
(myenv) pi@bpi-ai2n:~/project$ pip3 install Adafruit-Blinka
```
- Install from git source
+
```sh
(myenv) pi@bpi-ai2n:~/project$ pip3 install git+https://github.com/adafruit/Adafruit_Blinka
```

. Blinka Test
- The example test_blinka.py tests whether the Adafruit-Blinka package is installed correctly. Before running the test, please enable the i2c1 and spi2 DTB overlays.
+
```sh
import board
import digitalio
