...
Banana Pi BPI-AI2H System-on-Module (SoM) and BPI-AI2H carrier board are open-source hardware solutions designed to leverage the advanced capabilities of the RZ/V2H Vision AI MPU. It supports 8GB memory, 32GB emmc, dual gigabit ethernet ports, 2x USB3 interface.

== Specifications

- SoC – Renesas RZ/V2H quad-core 64bit Arm Cortex-A55 processor @ up to 1.8 GHz with Mali-G31 GPU
...
- USB – 3x USB 3.0 ports,1x USB 2.0 port,1x USB-C port
...
- Install Image with Armbian Imager on Windows, Linux and MacOS.
+
link:https://imager.armbian.com[Armbian Imager] is an open-source imaging tool for installing Armbian OS.
+
For un-official board images, please select Use Custom Image->Browse and select the local image file->Select the target storage->Erase&Flash.
+
image::/picture/armbian-imager-flash.png[armbian-imager-flash.png]
...
2. Copy target image to udisk, plug the udisk to board and mount it.
3. Install image to emmc.
...
More info about armbian build, please refer to link:https://docs.armbian.com/Developer-Guide_Build-Preparation/[Armbian Build Framework Quick Start Guide]
...
Armbian uses link:https://netplan.io/[Netplan.io] to describe networking configurations. Netplan is a utility to easily configure Linux networking, using a declarative approach. If you want to configure your network manually, it is as simple as editing and creating Netplan yaml files (see the yaml configuration reference at the link:https://netplan.readthedocs.io/en/stable/netplan-yaml/[Netplan docs]).

- Network-Manager Applet GUI
+
For Desktop images, you can right-click the NetworkManager applet icon in the system tray and select the "Edit Connection" item to configure the network.
+
image::/picture/ai2n-eth.png[ai2n-eth.png]
+
Connect to wireless network, you can left-click the NetworkManager applet icon in the system tray and select Available networks items to bring up a list of available wireless networks. 
+
image::/picture/ai2n-wifi-list.png[ai2n-wifi-list.png]
+
Click the network that you want to connect to. If the network is secured, a dialogue box will prompt you to enter the network key:
+
...
- Netplan
+
The Desktop and CLI images come with a default Netplan YAML file located at etc/netplan/00-default-use-network-manager.yaml. These images use the NetworkManager backend by default. All Ethernet interfaces are configured for DHCP and will automatically obtain an IP address from your router.
+
```sh
# Added by Armbian
#
...
```
If a Wi-Fi connection is successfully established during the first boot, a Wi-Fi Netplan YAML file will be created at /etc/netplan/30-wifis-dhcp.yaml. If no Wi-Fi connection is made during the first boot, this file can also be created manually after booting.
+
```sh
...
```
+
More info of Netplan configuration, please refer to the link:https://netplan.readthedocs.io/en/latest/using-static-ip-addresses[Netplan docs]

+
After YAML file changed, you should apply the configuration
+
```sh
...
- Command Line configure wireless connection
+
...
+
Run the following command to configure a network connection, replacing the <example_ssid> placeholder with the name of the network you’re trying to configure. Then enter your network password when prompted, the board should automatically connect to the network once you enter your password.
+
```sh
...
- Alternatively， You can also use **armbian-config** to configure networking configuration on all Armbian images.
+
```sh
...
The RZ/V2H chip features a single MIPI DSI interface for display output. We have developed a DSI-to-HDMI adapter board and a 7-inch HC070 Touch Panel board. 

- HDMI
+
Connect the HDMI bridge board to the carrier board’s MIPI DSI interface using an FPC cable, It supports resolutions up to 1080p@60Hz.
+
image::/picture/ai2n-hdmi.png[ai2n-hdmi.png]

- Lcd Panel
+
Connect the Bananapi 7" HC070 touch panel to the carrier board's MIPI DSI interface via an FPC cable. Enable panel-hc070iy30025 device-tree overlay.
+
```sh
...
== ADC
There have 3 units 12-bit adc in RZV2H, up to 8 channels per unit. The board adc pin header CN3 connected to adc1.

Read the adc data from iio sysfs
...
| HDParm      | Disk Read        | 12.20 MB/s            | 47.37 MB/s        | 806.92 MB/s               
| HDParm      | Cached Disk Read | 12.49 MB/s            | 41.18 MB/s        | 793.54 MB/s               
| DD          | Disk Write       | 14.4 MB/s             | 66.6 MB/s         | 168 MB/s                  
| FIO         | 4k random read   | 2328 IOPS (9313 KB/s) | 6623 IOPS (26494 KB/s) | 40715 IOPS (219037 KB/s) 
| FIO         | 4k random write  | 944 IOPS (3777 KB/s)  | 6450 IOPS (25801 KB/s) | 27713 IOPS (103173 KB/s)  
| IOZone      | 4k read          | 11684 KB/s            | 20569 KB/s        | 109433 KB/s               
| IOZone      | 4k write         | 4969 KB/s             | 19340 KB/s        | 66082 KB/s                
| IOZone      | 4k random read   | 8902 KB/s             | 20556 KB/s        | 42249 KB/s                
| IOZone      | 4k random write  | 5135 KB/s             | 23332 KB/s        | 117361 KB/s                
|             |                  | Score: 1371           | Score: 5146       | Score: 21261 
|=====
...
For detailed Wi-Fi configuration, please refer to the Networking section.
...
- Bluetooth using on Desktop image with Blueman
+
Right-click the bluetooth icon in the system tray and select Device item to the Bluetooth Device window.
+
image::/picture/ai2n-bt-blueman.png[ai2n-bt-blueman.png]

- Bluetooth using with bluetoothctl command
+
Install Bluez
+
```sh
...
== Type-C OTG
The USB Type-c port Only support device mode.
...
It already supports the RZ/V2H chip and the BPI-AI2H board. Dtb overlay must be enabled before using 40pin header bus control
...
+
Installing the library from git repo, this repo forked from link:https://github.com/adafruit/Adafruit_CircuitPython_SSD1306[Adafruit_CircuitPython_SSD1306], libs and examples modified to match the hardware link:https://docs.banana-pi.org/en/BPI-OLED/BananaPi_BPI-OLED[SSD1306 SPI Oled Module].
+
```sh
...
- blinkall: blink leds connected to all gpio pins, no extend board.
...
Note: This WiringPi only support set 40pin gpio to output, input, for io functions as i2c, spi, pwm..., you must enable dtb overlays
...
3. Install with dd command, umount mmcblk0p1 and mmcblk0p2 partition if mounted automatically. 
+
```sh
 $ sudo apt-get install gunzip pv
 $ sudo sh -c 'gunzip -c core-image-weston-bananapi-ai2h-xxx.rootfs.wic.gz | pv | dd of=/dev/sdX bs=1M status=progress && sync'
...
link:https://github.com/BPI-SINOVOIP/meta-bananapi/blob/bpi-ai2n/README.md[Build Guide]

== Yocto AI SDK and AI Application

link:https://renesas-rz.github.io/rzv_ai_sdk/6.00/ai-sdk.html[Renesas Wiki]

= Other Development

== Flash Spi Bootloader

The SPI bootloader consists of three parts: the Flash Writer file, ATF firmware, and U-Boot. There are three files: Flash_Writer*.mot, bl2_bp_spi.srec(bl2_bp_spi.bin), and fip.srec(fip.bin), which can be found in the /boot directory of the Linux image.

- Flash bootloader via the serial port on Windows PC

1. Copy Flash_Writer*.mot, bl2_bp_spi.srec and fip.srec to your Windows PC.
2. Connect Windows PC to Board via usb cabe to usb typec uart port bellow core board.
3. Change Bootsel Switch to SCIF download mode.
+
image::/picture/ai2n-boot-scidownload.png[ai2n-boot-scidownload.png]

4. On Windows PC, open the terminal emulator. Here, we use link:https://github.com/TeraTermProject/teraterm/releases/tag/v4.108[Tera Term 4.108] as an example.
+
Note: Select "Send in same way as Tera Term 4" in Send file Window if Tera Term 5.x version used.
+
image::/picture/ai2n-spiflash-teraterm5.png[ai2n-spiflash-teraterm5.png]
5. Select "File" > "New Connection" and select "Serial" port as shown below.
+
image::/picture/ai2n-spiflash-serial1.png[ai2n-spiflash-serial1.png]
6. Open the configuration window from the "Setup">"Terminal" and change the setting as follows.
+
image::/picture/ai2n-spiflash-serial2.png[ai2n-spiflash-serial2.png]
7. Open the configuration window from the "Setup">"Serial port" and change the setting as follows.
+
image::/picture/ai2n-spiflash-serial3.png[ai2n-spiflash-serial3.png]
8. Connect the power cable to power on the board and following message will be displayed on the terminal.
+
```sh
SCI Download mode (Normal SCI boot)
-- Load Program to SRAM ---------------
```
9. Open "File" > "Send file..." and send the Flash Writer file (*.mot) as a text. If following message is displayed, the file transfer succeeded.
+
```sh
...
>
```
10. Enter "XLS2" on the terminal to get following messages.
+
```sh
> XLS2
...
===== Please Input Program Top Address ============
  Please Input : H'
```
11. Enter "8101e00". The log continues.
+
```sh
...
===== Please Input Qspi Save Address ===
  Please Input : H'
```
12. Enter "00000". The log continues.
+
```sh
  Please Input : H'00000
please send ! ('.' & CR stop load)
```
13. After the "please send!" message, open "File" > "Send file..." and send the bl2_bp_spi-rzv2*.srec file as a text from the terminal software.
14. In case a message to prompt to clear data like below, please enter "y".
+
```sh
SPI Data Clear(H'FF) Check : H'00000000-0000FFFF,Clear OK?(y/n)
```
15. Following log will be displayed. The end address is depending on the version of AI SDK.
+
```sh
...
===========================================================
```
16. Enter "XLS2" on the terminal to get following messages.
+
```sh
> XLS2
===== Qspi writing of RZ/V2x Board Command =============
...
===== Please Input Program Top Address ============
  Please Input : H'
```
17. Enter "00000". The log continues.
+
```sh
  Please Input : H'00000
===== Please Input Qspi Save Address ===
  Please Input : H'
```
18. Enter "60000". The log continues.
+
```sh
  Please Input : H'60000
please send ! ('.' & CR stop load)
```
19. After the "please send!" message, open "File" > "Send file..." and send the fip-rzv2*.srec file as a text from the terminal software.
20. In case a message to prompt to clear data like below, please enter "y".
+
```sh
SPI Data Clear(H'FF) Check : H'00000000-0000FFFF,Clear OK?(y/n)
```
21. Following log will be displayed. The end address is depending on the version of AI SDK.
+
```sh
Write to SPI Flash memory.
======= Qspi Save Information =================
...
===========================================================
```
22. Power-off the board and change Bootsel Switch to default Boot Mode for booting the board.
+
image::/picture/ai2n-boot-spiboot.png[ai2n-boot-spiboot.png]

- Flash bootloader via the serial port on Linux PC

- Flash bootloader on AI2H board

== Erase emmc
Bootup the board with sdcard, erase the emmc with dd command as follows.
```sh
...
- link:https://www.waveshare.com/jetson-nano-dev-kit-a.htm?sku=21650[Waveshare JETSON-IO-BASE-A carrier board]
+
Edit /boot/armbianEnv.txt in armbian image or /boot/env.txt in yocto image as follow.
+
```sh
...
```
Note: 
1. HDMI and DP are not support by AI2H hardware. 
2. JETSON-IO-BASE-A use 5V DC, not 12V.
